...
const PricingCard = ({ 
  ...
  return (
    <div 
      ...
      className={`rounded-2xl shadow-xl overflow-hidden transform transition-all duration-300 hover:scale-105 hover:border-indigo-500 hover:border-4 cursor-pointer ${
        isPopular ? 'border-4 border-indigo-500 relative bg-white' : 'border border-gray-200 bg-white'
      } ${isSelected ? 'border-4 border-indigo-600 bg-indigo-50' : ''}`}
    >
      {isPopular && (
        <div className="absolute top-0 right-0">
          <div className="bg-indigo-600 text-white text-xs font-bold px-3 py-1 rounded-bl-lg shadow-lg">
        ...
        <button 
          className={`w-full py-3 px-6 rounded-xl font-semibold transition-all duration-300 
            ${isPopular 
              ...
              : 'bg-gray-100 hover:bg-indigo-600 hover:text-white text-indigo-600 border border-indigo-600'
            }`}
        >
          {buttonText}
        </button>
